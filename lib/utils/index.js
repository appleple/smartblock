...
import { Slice, Fragment, NodeRange, DOMSerializer } from 'prosemirror-model';
...
export var getRootNodeWithPosByIndex = function (state, index) {
    var i = 0;
    var firstNode = findChildren(state.doc, function () {
        if (i === index) {
            i++;
...
export var getRootNodeCountFromState = function (state) {
...
export var getParentNodeIndexFromState = function (state) {
    var selection = state.selection;
    var $anchor = selection.$anchor;
    var resolvedPos = state.doc.resolve($anchor.pos);
    var rowNumber = resolvedPos.path[1];
...
export var getParentNodeFromState = function (state) {
    var firstNode = getParentNodeWithPosFromState(state);
