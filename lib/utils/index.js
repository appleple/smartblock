...
var getOffset = function (el) {
    ...
    return {
        top: rect.top + (0, exports.getScrollTop)(),
        left: rect.left + (0, exports.getScrollLeft)()
    };
};
...
var getViewport = function () {
    ...
    if (/iPod|iPhone|iPad/.test(navigator.platform) &&
        (0, exports.isInput)(document.activeElement)) {
...
var markActive = function (type) { return function (state) {
...
var getMarkInSelection = function (markName, state) {
    var selection = state.selection;
    var $anchor = selection.$anchor, $head = selection.$head;
    // Helper function to find the mark in the node
    var findMark = function (node) {
        if (node && node.marks) {
            return node.marks.find(function (mark) { return mark.type.name === markName; });
        }
        return null;
    };
    // Check the mark at the $anchor position
    var mark = findMark($anchor.nodeAfter);
    if (mark) {
        return mark;
    }
    ...
    var from = Math.min(selection.from, selection.to);
    var to = Math.max(selection.from, selection.to);
    var rangeHasMark = false;
    state.doc.nodesBetween(from, to, function (node) {
        if (node.marks.some(function (mark) { return mark.type.name === markName; })) {
            rangeHasMark = true;
        }
    });
    if (rangeHasMark) {
...
exports.getMarkInSelection = getMarkInSelection;
var blockActive = function (type) { return function (state) {
    var selection = state.selection;
...
var canInsert = function (type) { return function (state) {
...
var getRootNodeWithPosByIndex = function (state, index) {
    var i = 0;
    var firstNode = (0, prosemirror_utils_1.findChildren)(state.doc, function () {
        if (i === index) {
            i++;
...
exports.getRootNodeWithPosByIndex = getRootNodeWithPosByIndex;
var getRootNodeCountFromState = function (state) {
    return state.doc.content.childCount;
};
exports.getRootNodeCountFromState = getRootNodeCountFromState;
var getParentNodeWithPosFromState = function (state) {
    var selection = state.selection;
    var $anchor = selection.$anchor;
    var resolvedPos = state.doc.resolve($anchor.pos);
    var rowNumber = resolvedPos.path[1];
...
exports.getParentNodeWithPosFromState = getParentNodeWithPosFromState;
var getParentNodeIndexFromState = function (state) {
    var selection = state.selection;
    var $anchor = selection.$anchor;
    var resolvedPos = state.doc.resolve($anchor.pos);
    var rowNumber = resolvedPos.path[1];
...
exports.getParentNodeIndexFromState = getParentNodeIndexFromState;
var getParentNodeFromState = function (state) {
    var firstNode = (0, exports.getParentNodeWithPosFromState)(state);
    var node = firstNode.node;
    return node;
};
exports.getParentNodeFromState = getParentNodeFromState;
var getParentNodePosFromState = function (state) {
    var node = (0, exports.getParentNodeFromState)(state);
    var pos = (0, exports.findNodePosition)(state.doc, node);
    return pos + node.nodeSize;
};
exports.getParentNodePosFromState = getParentNodePosFromState;
var findSelectedNodeWithType = function (nodeType, state) {
    var _a = state.selection, from = _a.from, to = _a.to;
...
exports.findSelectedNodeWithType = findSelectedNodeWithType;
function liftToOuterList(state, dispatch, itemType, range) {
    ...
    dispatch(tr.lift(range, (0, prosemirror_transform_1.liftTarget)(range) - 1).scrollIntoView());
    return true;
}
...
var liftListItem = function (itemType) {
...
var createTable = function (schema, attrs, rowsCount, colsCount, withHeaderRow, cellContent) {
...
var calculateStyle = function (view, offset) {
    ...
    var elementTop = (0, exports.getOffset)(element).top;
    var coords = view.coordsAtPos(selection.$anchor.pos);
    var offsetTop = (0, exports.getOffset)(view.dom).top;
...
exports.stripPtag = stripPtag;
var getHtmlFromNode = function (doc, schema) {
    var fragment = prosemirror_model_1.DOMSerializer.fromSchema(schema).serializeFragment(doc.content);
    var div = document.createElement('div');
    div.appendChild(fragment);
    return (0, exports.stripPtag)(div.innerHTML);
};
exports.getHtmlFromNode = getHtmlFromNode;
